    ...
    for dir_path, dirs, files in os.walk(src_path):
        for file_obj in files:
            if file_obj.upper().startswith(file_name) and not file_obj.endswith(".py"):
                ...
                print("copy file from "+  dir_path+"\\"+file_obj   +    " to "+ dst_path + "\\"+ file_obj)
                found = True
...
path_prefix = os.path.join(os.environ.get("BUILD_SOURCESDIRECTORY"),r"cmake_build\release")
...
for component in special_components:
    ...
    os.mkdir(dst_dir)
    shutil.copyfile(os.path.join(os.environ.get("BUILD_SOURCESDIRECTORY"), r"tensorflow\tools\ci_build\windows\nuget\manual_collected_license", component+".txt"), os.path.join(dst_dir, "LICENSE"))
